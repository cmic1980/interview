...
public class SocketChannelEchoServer {
    ...
    public static void main(String[] args) {
        SocketChannelEchoServer echoServer = new SocketChannelEchoServer();
        echoServer.init();
    }
    ...
    private void initServerSocketChannel() {
        try {
            ...
            server.socket().bind(new InetSocketAddress(ServerInfo.SERVER_PORT), 1024);
    ...
    private void handelReadable(SelectionKey key) throws IOException {
        ...
        int position = ssc.read(buffer); // 从channel读到buffer
        while (position != 0) {
